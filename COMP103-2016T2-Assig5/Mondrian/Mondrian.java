...
public class Mondrian {
    ...
    public void drawMondrian(int x1, int y1, int x2, int y2, int currentLevel) {
        ...
        if((currentLevel != 0 && chance != 0 && (x2-x1) > MondrianSideMinimum && (y2-y1) > MondrianSideMinimum )){
            int i=0;
            int j=0;
            while(i<25||i>75){ // random the sprite pointX between 25%~75%
                i= (int)(Math.random()*100);
            }
            while(j<25||j>75){ // random the sprite pointY between 25%~75%
                j= (int)(Math.random()*100);
            }
            
            int spX=(x2-x1)*i/100+x1;   //calculate the sprite point X and Y
            int spY=(y2-y1)*j/100+y1;  
            
            UI.setColor(mondrianBlack); //draw the rectangles 
            UI.drawRect(x1, y1, spX-x1, spY-y1); // Left Top
            UI.drawRect(spX, y1, x2-spX, spY-y1 ); // Right Top
            UI.drawRect(x1, spY, spX-x1, y2-spY);  // Left Bot
            UI.drawRect(spX, spY, x2-spX, y2-spY); // Right Bot
            
            //Call methods with the new sprite point
            if ((int)(Math.random()*100) < this.chance)  // Left Top
                drawMondrian(x1, y1, spX, spY, currentLevel-1);
            
            if ((int)(Math.random()*100)< this.chance)   // Right Top           
                drawMondrian(spX, y1, x2, spY, currentLevel-1);
            
            if ((int)(Math.random()*100)< this.chance)   // Left Bot             
                drawMondrian(x1, spY, spX, y2, currentLevel-1);
              
            if ((int)(Math.random()*100) < this.chance)  //Right Bot         
                drawMondrian(spX, spY, x2, y2, currentLevel-1);
