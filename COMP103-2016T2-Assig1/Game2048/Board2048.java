...
public class Board2048 {
    ...
    public boolean isGameOver() {
        ...
        for(int row = 0; row < ROWS-1; row++){
            for (int col = 0; col < COLUMNS-1; col++) {
                ...
                double rightvalue = board[row][col+1];
                double botvalue = board[row+1][col];
                double RBvalue = board[row+1][col+1];

                if((board[row][col]!=rightvalue&&board[row][col]!=botvalue)&&(botvalue!=RBvalue&&rightvalue!=RBvalue)&&(numEmptyTiles()==0)){
                    count++;
                }
                if(count==9){
                    return true;
    ...
    public void left() {
        ...
        for(int rows = 0; rows < ROWS; rows++){
            for (int cols = 0; cols < COLUMNS-1; cols++) { // the loop for shift empty tiles
                int [][] tempboard = new int [ROWS][COLUMNS]; // make a new tempboard

                for(int row = 0; row < ROWS; row++){
                    int count = 0;
                    for (int col = 0; col < COLUMNS; col++) {
                        if(board[row][col]!=0){ //cheak the value that are 2,4,8,16...
                            tempboard[row][count]=board[row][col];//move them into the new array
                            count++;
                        } 
                    }
                }
                ...
                for(int row = 0; row < ROWS; row++){
                    for (int col = 0; col < COLUMNS; col++) {
                        board[row][col]=tempboard[row][col]; //let the temp array become the new array
                    }
                }

                double nextvalue = board[rows][cols+1];
                if(nextvalue==board[rows][cols]){ //Shift all non-empty tiles
    ...
    public void right() {
        ...
        for(int rows = 0; rows < ROWS; rows++){
            for (int cols = COLUMNS-1; cols>0;cols--){
                ...
                for(int row = 0; row < ROWS; row++){
                    ...
                    for (int col = COLUMNS-1; col>=0 ; col--) {
                        if(board[row][col]!=0){
                            tempboard[row][count]=board[row][col];
                            count--;
                        } 
                    }
                }
                ...
                double lastvalue = board[rows][cols-1];
                if(lastvalue==board[rows][cols]){
                    board[rows][cols]+=board[rows][cols-1];
                    board[rows][cols-1]=0;
                }
        ...
        for(int rows =0 ; rows < ROWS-1; rows++){
            for (int cols = 0; cols < COLUMNS; cols++) {
                ...
                for (int col = 0; col < COLUMNS; col++) {
                    ...
                    for(int row = 0; row < ROWS; row++){
                        if(board[row][col]!=0){
                            tempboard[count][col]=board[row][col];
                            count++;
                        } 
                    }
                ...
                double nextvalue = board[rows+1][cols];
                if(nextvalue==board[rows][cols]){
    ...
    public void down() {
        ...
        for(int rows =  ROWS-1; rows>0; rows--){
            for (int cols = 0; cols < COLUMNS; cols++) {
                ...
                double lastvalue = board[rows-1][cols];
                if(lastvalue==board[rows][cols]){
                    board[rows][cols]+=board[rows-1][cols];
                    board[rows-1][cols]=0;
                }
