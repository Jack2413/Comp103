...
public class Board16 {
    ...
    public boolean isGameOver() {
        ...
        for (int col = 1; col < COLUMNS-1; col++) {
            double Lastvalue = board[col-1];
            double nextvalue = board[col+1];
            if((board[col]!=Lastvalue&&board[col]!=nextvalue)&&(numEmptyTiles()==0)){
                count++;
                if(count==3){
                    return true;
                }
    ...
    public void left() {
        ...
        for (int cols = 1; cols < COLUMNS; cols++) {
            ...
            for (int col = 0; col < COLUMNS; col++) {
                if(board[col]!=0){
                    tempboard[count]=board[col]; 
                    count++;
                } 
            }

            for (int col = 0; col < COLUMNS; col++) {
                board[col]=tempboard[col];
            }
            double lastvalue = board[cols-1];
            if(lastvalue==board[cols]){
                board[cols-1]+=board[cols];
                board[cols]=0;
            }
        }
