...
public class GeneralTree
{
    ...
    public void addNode(String newName, String parentName) {
        ...
        else if(findNode(parentName) != null && findNode(newName) == null){

            child=new GeneralTreeNode(newName);
            findNode(parentName).addChild(child);
        }
        else if( findNode(newName) != null){
            UI.println("duplicate is not allow");
        }
        else{
            UI.println("can't find parent");
    ...
    public void removeNode(String targetName) {
        if(findNode(targetName)==null||root.getName()==targetName)return;
        else
            findNode(targetName).remove();
    ...
    public void moveSubtree(String targetName, String destinationName) {
        ...
        if(targetNode.contains(destinationNode))return;
    ...
    public String findClosestCommonAncestor(String target1, String target2) {
        if(findNode(target1).contains(findNode(target2))) return target1;
        else{ String Ancestor = findClosestCommonAncestor( findNode(target1).getParent().getName(),target2);
