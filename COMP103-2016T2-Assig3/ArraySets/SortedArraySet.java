...
public class SortedArraySet <E extends Comparable> extends AbstractSet <E> {
    ...
    public SortedArraySet() {
        data=(E[])new String[INITIALCAPACITY];
    }
    ...
    */
    ...
    public boolean remove (Object item) {
        ...
        if(contains(item)&&!item.equals(null)){
            E itm = (E) item;
            int i = findIndexOf(itm);
            if(itm!=null){
                if(data[i]!=null){

                    if(item!=null&&data[i].equals(item)){
                        data[i]=null;
                        for(int j= i ;j<count-1;j++){
                            data[j]=data[j+1];              
                        }
    ...
    */
    ...
    private void ensureCapacity () {
        ...
        E[] newdata=(E[])new String[data.length*2];
    ...
    */
    ...
    private int findIndexOf(E item) {
        ...
        while(low<=high){
            ...
            if(data[mid]==null) return 0;//when adds the frist item retrun 0, so not retrun null.
