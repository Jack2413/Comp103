...
public class BellsPizzaSimulation {
    ...
    private List <Integer> arriveTime =new <Integer> ArrayList();
    private int earliest=100;
    ...
    private double run (Queue<Pizza> queue, Scanner logFile) {
         ...
         */
        int numberOfAvailableCyclist=3;

        int cyclistAvailableAt = 0;
         // the time when the cyclist will next be available.
        ...
        while (logFile.hasNext()) {
            ...
            if ((time>earliestCyclistAvailableAt()||numberOfAvailableCyclist>0) && !queue.isEmpty()) {

                // obtain next pizza from the queue
                Pizza deliveredPizza = queue.poll();
                numberOfAvailableCyclist--;
                
               //UI.println(earliestCyclistAvailableAt());
                if(arriveTime.size()!=0  &&  numberOfAvailableCyclist==0)  arriveTime.remove(LocationOfearliest(earliest));

                

                // is it impossible to deliver this pizza on time?
                if (time + deliveredPizza.deliveryTime() > deliveredPizza.deadline()&&numberOfAvailableCyclist==0) {
                ...
                arriveTime.add(time + fullTripTime);
    ...
    public int earliestCyclistAvailableAt(){
        
        for(int i=0; i< arriveTime.size(); i++){
            earliest=Math.min(earliest, arriveTime.get(i));
            //UI.println(arriveTime.get(i));
            //UI.println();
    ...
    public int LocationOfearliest(int earliest){
        for(int i=0; i< arriveTime.size(); i++){
            if(arriveTime.get(i)==earliest)return i;
