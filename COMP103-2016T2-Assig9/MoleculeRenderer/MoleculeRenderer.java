...
public class MoleculeRenderer {
    ...
    public MoleculeRenderer() {
        ...
        UI.addButton("FromLeft", () -> view(90, new LeftToRightComparator()));
        UI.addButton("FromRight", () -> view(-90, new RightToLeftComparator()));
        UI.addButton("Rotate right", () -> changeView(-1));
        UI.addButton("Rotate left", () -> changeView(1));

        UI.addSlider("Zoom out  /  Zoom In", 1, 20, this::Zoom);
        ...
        view(currentAngle += (value * rotationStep), new PerspectiveComparator());
        render();
    }
    ...
    private void Zoom(double value) {
        ...
        UI.getGraphics().scale(1+(value-C_value)/10,1+(value-C_value)/10);
        C_value=value;
    ...
    public void readMoleculeFile(String fname) {
        ...
        try {
            ...
            while (scan.hasNext()) {
                AtomInfo cur = atoms.get(scan.next());
                molecule.add(new MoleculeElement(scan.nextDouble(), scan.nextDouble(), scan.nextDouble(), cur.color, cur.radius));
                scan.nextLine();
    ...
    private void readAtomInfos() {
        ...
        try {
            ...
            while (scan.hasNext()) {
                ...
                atoms.put(name, new AtomInfo(name, scan.nextInt(), new Color(scan.nextInt(), scan.nextInt(), scan.nextInt())));
            }
