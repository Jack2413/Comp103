...
public class Sokoban {
    ...
    private Stack<Boolean>boxmove= new Stack<Boolean>();

    private Stack<String>redo= new Stack<String>();

    private ArrayList<String> cheaked;
    private ArrayList<String> key;
    private Map<Square[][], Boolean> cheak;
    ...
    public Sokoban() {
        ...
        UI.println("You may use keys (wasd or ijkl a?nd u)");
    ...
    public void Undo(){
        ...
        if (!boxmove.isEmpty()&&!dir.isEmpty()&&boxmove.peek()) {
            pull(oppositeDirection(dir.peek()));
        ...
        else if(!boxmove.isEmpty()&&!dir.isEmpty()){ // can the worker move?
            move( oppositeDirection(dir.peek()));
    ...
    public void Redo(){
        if(!redo.isEmpty()){
            ...
            UI.println("can't redo anymore");        
        }        
    }

    public void doMouse(String mouseAction, double x, double y) {
        //where the mouse was pressed
        //set boolean cheak not cheaked
        //findpath around the squares

        if (mouseAction.equals("pressed")){
            int rowt=(int)((x-40)/25);
            int colt=(int)((y-40)/25);
            int stepX;
            int stepY;
            UI.println(rowt);
            UI.println(colt);
            if (squares[rowt][colt].isFree()){
                key = new ArrayList<String>();

                UI.println(workerPosition);
                //stepX=pressedX-row;
                //stepY=pressedY-col;
                while(true){

                    if(squares[rowt][colt]!=squares[workerPosition.row][workerPosition.col]){

                        if(squares[rowt+1][colt].isFree()){
                            key.add("right");
                            rowt+=1;

                        }
                        else if(squares[rowt-1][colt].isFree()){
                            key.add("left");
                            rowt-=1;
                        }
                        else if(squares[rowt][colt+1].isFree()){
                            key.add("down");
                            colt+=1;
